...
########################################################
## Extended Data Figure 2A & 2B - PYL1 synonymous WTs ##
########################################################
...
## Libraries
packages <- c("stringr", "drc", "reshape", "ggplot2", "ggtext", "scales", "beeswarm", "rlang")
...
pdf("../../results/FigureED2/FigureED2A_synonymous_WT_dose_response_curves.pdf", 
    height = 15, width = 18)
...
out.S2A <- ggplot(data = PYL1.ABI1.WT.curves) +
  ...
  annotate("text",
           ...
           y = 105,
           label = expr_text(bquote(italic(N) == .(format(length(PYL1.ABI1.WT.ls), digits = 2)))),
           parse = T,
...
### B[0]
pdf('../../results/FigureED2/FigureED2B_B0.pdf', 
    width = 6, height = 12)
...
axis(2, 
     ...
     las = 2,
     line = 1)
mtext(text = bquote(B[0] ~ "(Relative PYL1/ABI1 Binding)"), side = 2, line = 10, cex = 3)

dev.off()

### B[inf]
pdf('../../results/FigureED2/FigureED2B_Binf.pdf', 
    width = 6, height = 12)
...
axis(2, 
     ...
     las = 2,
     line = 1)
mtext(text = bquote(B[infinity] ~ "(Relative PYL1/ABI1 Binding)"), side = 2, line = 10, cex = 3)

dev.off()

## EC50
pdf('../../results/FigureED2/FigureED2B_EC50.pdf', 
    width = 6, height = 12)
...
mtext(text = bquote(EC[50] ~ "((+)-ABA, µM)"), side = 2, line = 10, cex = 3)

dev.off()

## Hill
pdf('../../results/FigureED2/FigureED2B_Hill.pdf', 
    width = 6, height = 12)
...
axis(2, 
     ...
     las = 2,
     line = 1)
mtext(text = "Hill coefficient", side = 2, line = 10, cex = 3)
...
# [1] rlang_1.1.6    beeswarm_0.4.0 scales_1.4.0   ggtext_0.1.2   ggplot2_4.0.0  reshape_0.8.10 drc_3.0-1      MASS_7.3-65    stringr_1.5.2 
# 
# loaded via a namespace (and not attached):
# [1] Matrix_1.7-4       gtable_0.3.6       crayon_1.5.3       dplyr_1.1.4        compiler_4.5.1     gtools_3.9.5       tidyselect_1.2.1   plotrix_3.8-4      Rcpp_1.1.0         xml2_1.4.0        
# [11] splines_4.5.1      lattice_0.22-7     TH.data_1.1-4      R6_2.6.1           plyr_1.8.9         generics_0.1.4     Formula_1.2-5      tibble_3.3.0       car_3.1-3          pillar_1.11.1     
# [21] RColorBrewer_1.1-3 multcomp_1.4-28    stringi_1.8.7      S7_0.2.0           cli_3.6.5          withr_3.0.2        magrittr_2.0.4     gridtext_0.1.5     grid_4.5.1         mvtnorm_1.3-3     
