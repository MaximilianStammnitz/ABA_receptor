...
###########################################################
## Extended Data Figure 6B - aPCA residual volcano plots ##
###########################################################
...
p0 <- ggplot(loess.res.pos, aes(x = B0, y = -log10(`B0 FDR`))) +
  ...
  geom_text_repel(data = loess.res.pos[grep("T33|E36|F37|Q39|S43|R143|V177|P178", rownames(loess.res.pos)),],
                  aes(label = rownames(loess.res.pos[grep("T33|E36|F37|Q39|S43|R143|V177|P178", rownames(loess.res.pos)),])),
...
pEC50 <- ggplot(loess.res.pos, aes(x = EC50, y = -log10(`EC50 FDR`))) +
  ...
  scale_fill_gradient2(low = "red", mid = "gray90", high = "blue", 
                       ...
                       oob = squish) +
  geom_hline(yintercept = -log10(0.01), linetype = "dashed", color = "black", alpha = 0.2) +
  geom_text_repel(data = loess.res.pos[grep("L196|H87|Y50|K200|I111|E171|D107", rownames(loess.res.pos)),],
                  aes(x = log10(`EC50`), y = -log10(`EC50 FDR`),
                      label = rownames(loess.res.pos[grep("L196|H87|Y50|K200|I111|E171|D107", rownames(loess.res.pos)),])),
                  size = residue.label.size, 
                  family = "Helvetica",
                  max.overlaps = 1000,
                  force = 2,
...
pHill <- ggplot(loess.res.pos, aes(x = Hill, y = -log10(`Hill FDR`))) +
  ...
  scale_fill_gradient2(low = "red", mid = "gray90", high = "blue", 
                       ...
                       oob = squish) +
  geom_hline(yintercept = -log10(0.01), linetype = "dashed", color = "black", alpha = 0.2) +
  geom_text_repel(data = loess.res.pos[grep("S41|Q42|H154", rownames(loess.res.pos)),],
                  aes(x = log10(`Hill`), y = -log10(`Hill FDR`),
                      label = rownames(loess.res.pos[grep("S41|Q42|H154", rownames(loess.res.pos)),])),
                  size = residue.label.size, 
                  family = "Helvetica",
                  max.overlaps = 1000,
                  force = 2,
...
## plot all
pdf('../../results/FigureED6/FigureED6B_Hill_paramater_residuals_volcanos.pdf', width = 12, height = 42)
plot_grid(p0, pinf, pEC50, pHill, align = "hv", axis = "tblr", ncol = 1)
dev.off()
