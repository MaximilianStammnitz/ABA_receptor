...
##############################################################################################
## Extended Data Figure 5C - LOESS fit of aPCA vs. all 12 GluePCA concentrations of (+)-ABA ##
##############################################################################################
...
## Libraries
packages <- c("readxl", "stringr", "scales", "reshape", "ggplot2", "ggtext", "ggrepel", "cowplot", "rlang")
...
## LOESS of all twelve comparisons (using "alive" aPCA values >5)
all.out.alive <- all.out[which(all.out$aPCA >5),]
...
loess.pred <- expand.grid("aPCA" = seq(f = 5, 
...
for (i in 1:12){
  ...
  plots.out[[i]] <- ggplot(all.out, mapping = aes(x = `aPCA`, y = .data[[name.tmp]])) +
    ...
    scale_x_continuous(breaks = seq(from = 0, to = 100, length.out = 6), limits = c(-20, 130)) +
    scale_y_continuous(breaks = seq(from = 0, to = 100, length.out = 6), limits = c(-20, 130)) +
    ...
    theme(plot.title = element_markdown(hjust = 0.5),
          ...
          axis.title.x = element_text(family = 'Helvetica', colour = 'black', size = 30, vjust = -1),
          axis.title.y = element_text(family = 'Helvetica', colour = 'black', size = 30, vjust = 3),
          legend.position = "none",
          text = element_text(family="Helvetica"),
          plot.margin = unit(c(2, 2, 2, 2),"cm")) +
    labs(x = "Relative PYL1 Abundance",
         y = "Relative PYL1/ABI1 Binding",
         title = paste0(format(round(dosages[i], 3), nsmall = 3), " µM (+)-ABA"))
  
}

pdf('../../results/FigureED5/FigureED5C_abundancePCA_vs_binding_series.pdf', width = 58, height = 20)
plot_grid(plots.out[[1]], plots.out[[2]], plots.out[[3]], 
          plots.out[[4]], plots.out[[5]], plots.out[[6]], 
          plots.out[[7]], plots.out[[8]], plots.out[[9]], 
          plots.out[[10]], plots.out[[11]], plots.out[[12]], 
          align = "hv", axis = "tblr", ncol = 6)
dev.off()
...
# [1] rlang_1.1.6    cowplot_1.2.0  ggrepel_0.9.6  ggtext_0.1.2   ggplot2_4.0.0  reshape_0.8.10 scales_1.4.0   stringr_1.5.2  readxl_1.4.5  
# 
# loaded via a namespace (and not attached):
# [1] gtable_0.3.6       dplyr_1.1.4        compiler_4.5.1     tidyselect_1.2.1   Rcpp_1.1.0         xml2_1.4.0         R6_2.6.1           plyr_1.8.9         commonmark_2.0.0   generics_0.1.4    
# [11] tibble_3.3.0       pillar_1.11.1      RColorBrewer_1.1-3 stringi_1.8.7      xfun_0.53          litedown_0.7       S7_0.2.0           cli_3.6.5          withr_3.0.2        magrittr_2.0.4    
# [21] grid_4.5.1         gridtext_0.1.5     markdown_2.0       lifecycle_1.0.4    vctrs_0.6.5        glue_1.8.0         farver_2.1.2       cellranger_1.1.0   tools_4.5.1        pkgconfig_2.0.3 