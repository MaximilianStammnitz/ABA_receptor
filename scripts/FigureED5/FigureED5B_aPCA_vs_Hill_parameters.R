...
#############################################################
## Extended Data Figure 5B - LOESS fit vs. Hill parameters ##
#############################################################
...
## Libraries
packages <- c("stringr", "scales", "reshape", "ggplot2", "hexbin", "ggtext", "ggrepel", "cowplot", "rlang")
...
p1 <- ggplot(Hill.df) +
  ...
  annotate("text",
           x = -20,
           y = 115,
           label = expr_text(bquote(rho == .(format(r.B0, nsmall = 2, digits = 2)))),
           parse = T,
           hjust = 0, size = 15, color = "black") +
  scale_x_continuous(breaks = seq(from = 0, to = 100, length.out = 6), limits = c(-20, 130)) +
  scale_y_continuous(breaks = seq(from = 0, to = 100, length.out = 6), limits = c(-20, 120)) +
  coord_cartesian(xlim = c(-20, 130), ylim = c(-20, 120)) +
  ...
  labs(x = "Relative PYL1 Abundance",
       y = bquote(B[0] ~ "(Relative PYL1/ABI1 Binding)"))
...
p2 <- ggplot(Hill.df) +
  ...
  annotate("text",
           x = -20,
           y = 115,
           label = expr_text(bquote(rho == .(format(r.Binf, nsmall = 2, digits = 2)))),
           parse = T,
           hjust = 0, size = 15, color = "black") +
  scale_x_continuous(breaks = seq(from = 0, to = 100, length.out = 6), limits = c(-20, 130)) +
  scale_y_continuous(breaks = seq(from = 0, to = 100, length.out = 6), limits = c(-20, 120)) +
  ...
  theme(plot.title = element_markdown(),
        ...
        plot.margin = unit(c(2, 2, 2, 2),"cm")) +
  labs(x = "Relative PYL1 Abundance",
       y = bquote(B[infinity] ~ "(Relative PYL1/ABI1 Binding)"))
...
p3 <- ggplot(Hill.df) +
  ...
  annotate("text", x = -20, y = 10000, 
           label = expr_text(bquote(rho == .(format(r.EC50, nsmall = 2, digits = 2)))), 
           parse = T,
           hjust = 0, size = 15, color = "black") +
  theme_classic(base_size = 50) +
  theme(plot.title = element_markdown(),
...
p4 <- ggplot(Hill.df) +
  ...
  annotate("text", x = -20, y = 3, 
           label = expr_text(bquote(rho == .(format(r.n, nsmall = 2, digits = 2)))), 
           parse = T,
           hjust = 0, size = 15, color = "black") +
  theme_classic(base_size = 50) +
  theme(plot.title = element_markdown(),
...
pdf('../../results/FigureED5/FigureED5B_abundancePCA_vs_Hill_parameters.pdf', width = 45, height = 13)
...
# [1] rlang_1.1.6    cowplot_1.2.0  ggrepel_0.9.6  ggtext_0.1.2   hexbin_1.28.5  ggplot2_4.0.0  reshape_0.8.10 scales_1.4.0   stringr_1.5.2 
# 
# loaded via a namespace (and not attached):
# [1] vctrs_0.6.5        cli_3.6.5          stringi_1.8.7      generics_0.1.4     S7_0.2.0           labeling_0.4.3     glue_1.8.0         plyr_1.8.9         gridtext_0.1.5     grid_4.5.1        
# [11] tibble_3.3.0       lifecycle_1.0.4    compiler_4.5.1     dplyr_1.1.4        RColorBrewer_1.1-3 Rcpp_1.1.0         pkgconfig_2.0.3    farver_2.1.2       lattice_0.22-7     R6_2.6.1          
